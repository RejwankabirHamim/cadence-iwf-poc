package kubevirt_cluster_create
...
func NewKubevirtClusterCreateWorkflow(svc service.MyService) iwf.ObjectWorkflow {

	return &KubevirtClusterCreateWorkflow{
		svc: svc,
	}
}

type KubevirtClusterCreateWorkflow struct {
	iwf.WorkflowDefaults

	svc service.MyService
}

func (e KubevirtClusterCreateWorkflow) GetWorkflowStates() []iwf.StateDef {
	return []iwf.StateDef{
...
func (i createNamespaceState) Execute(
	...
	input.Get(&request)
	nsname := fmt.Sprintf("%s-%s", operation.GetCAPIConfig().ClusterName, rand.String(6))
	...
	return iwf.SingleNextState(&CreateJobState{}, request), nil
}
